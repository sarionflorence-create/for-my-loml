...
document.querySelectorAll('.open-letter-btn').forEach((btn) => {
  btn.addEventListener('click', () => {
    const letterBox = btn.nextElementSibling;
    const fullText = letterBox.getAttribute("data-letter");

    // If letter is currently hidden = show and type it
    if (letterBox.classList.contains("hidden")) {
      ...
      let i = 0;
      let speed = 30; // adjust speed here (smaller = faster)

      function typeWriter() {
        if (i < fullText.length) {
          letterBox.textContent += fullText.charAt(i);
          i++;
